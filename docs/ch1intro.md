...
Examples of this include, but are not limited to: keeping track of stabilizer frames rather than applying recovery operations directly, doing try-until-success operations for things like logical state preparation, running additional rounds of syndrome extraction if errors are detected, and quantum operations that involve physical qubits outside of logical qubits, such as split/merge operations in lattice surgery.
