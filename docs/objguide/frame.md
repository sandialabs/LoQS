...
As we have already seen in the [History tutorial](/objguide/history), a `Frame` can be created from any `collections.abc.Mapping` object, most often a `dict`.
...
See the [API Reference](/devguide/_autosummary/loqs.core.frame.Frame) for more in-depth documentation of `Frame` objects.
