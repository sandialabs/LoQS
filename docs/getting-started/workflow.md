...
The individual `History` objects are now available in the `ProgramResults` object returned by `program.run()`.
...
stack_1 = [('Init State', None, (len(qubits),), {'qubit_labels': qubits}), ('Init Patch 5Q', None, ('L0', qubits)), ('Non-FT Minus Prep', 'L0'), ('H', 'L0'), ('X', 'L0'), ('Non-FT Logical Z Measure', 'L0')]
program2 = QuantumProgram.from_quantum_program(program, stack_1, name='Prep 0, measure Z')  # Autogenerated from state_type
results2 = program2.run(num_shots=100)  # Autogenerated from patch_types. Note that 5Q here must be a key
# We want to use the same program settings, just swap out the stack and name
Counter(results2.collect_shot_data('logical_measurement', -1))  # Rotate to logical Z basis  # Flip from -1 eigenstate to +1 eigenstate
```
...
stack_2 = [('Init State', None, (len(qubits),), {'qubit_labels': qubits}), ('Init Patch 5Q', None, ('L0', qubits)), ('Non-FT Minus Prep', 'L0'), ('Non-FT Logical Z Measure', 'L0')]
program3 = QuantumProgram.from_quantum_program(program, stack_2, default_base_seed=20240702, name='Prep -, measure Z')  # Autogenerated from state_type
results3 = program3.run(num_shots=1000)  # Autogenerated from patch_types. Note that 5Q here must be a key
# This one will be non-determinate outcome. Let's seed the RNG
Counter(results3.collect_shot_data('logical_measurement', -1))  # Let's also collect more statistics!
```
...
results6 = program6.run(num_shots=1000, checkpoint_dir="test-checkpoint", override_checkpoints=True)
```

We can now load the program results using the ``ProgramResults.load_checkpoint`` method. Checking the number of shot histories, we should have as many as was completed before we interrupted the program above.

```python {marimo}
# Create a new ProgramResults object and load from checkpoint
results7 = ProgramResults()
results7.load_checkpoint('test-checkpoint')
```

```python {marimo}
len(results7.shot_histories)
```
...
# Note: This would require creating a new QuantumProgram and running it,
```