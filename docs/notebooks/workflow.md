...
The entries are `InstructionLabel`s, which are really just dicts with the following keys:

1. `"instruction"`: Either a key into `QuantumProgram.global_instructions` or a `QECCode.instructions` (discussed more below) or an actual `Instruction` object
2. `"patch_label"`: A patch label
3. Any other keyword arguments to pass into the `Instruction.apply_fn` (discussed in the [Instruction tutorial](/reference/loqs/core/instructions/instruction/Instruction/))

You'll notice that all of the instruction keys below are strings rather than an `Instruction`.
...
stack = [
    ...
    ("Non-FT Minus Prep", "L0"), # (instruction, patch_label) 2-tuple shorthand for {"instruction": ..., "patch_label": ...} when there are no other kwargs
...
stack = [
    {"instruction": "Init State", "state": len(qubits), "qubit_labels": qubits}, # Autogenerated from state_type
    {"instruction": "Init Patch 5Q", "new_patch_label": "L0", "qubits": qubits}, # Autogenerated from patch_types. Note that 5Q here must be a key
