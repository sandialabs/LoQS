...
The crux of the difference is that we want to make the model an "expiring" and "propogated" key in the ``Frame`` rather than be stored in the ``QuantumProgram``. See the relevant documentation in the [``History`` tutorial](/reference/loqs/core/history/History/), but the general thought process here:
...
For detailed information on building an `Instruction`, check out the section on [Instructions](/reference/loqs/core/instructions/instruction/Instruction/) and the (in progress) ["Building a Complex Instruction" tutorial](buildinstruction.md). However, the TL;DR of the other documentation is that an `Instruction` has four main parts: the apply function, data, the map qubits function, and parameter information. Let's lay out the API for our model update `Instruction`, and see how it lines up with what we need to define.
