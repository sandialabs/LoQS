...
# C901: nested loops over files, classes, and methods with branching introspection merging
def main() -> None:  # noqa: C901
    ...
    for py in sorted(PKG_DIR.rglob("*.py")):
        ...
        if py.name == "__init__.py":
            ...
            mod_ident = "loqs" + (
                "" if len(mod_parts) == 1 else "." + ".".join(mod_parts[1:])
            )
        else:
        ...
        for fn in funcs:
            inv_objects[f"{mod_ident}.{fn}"] = (
                f"{mod_page_url}#{mod_ident}.{fn}"
            )
        ...
        for cls_name in classes:
            ...
            if plan.cls_obj is not None:
                ...
                prop_rows = property_rows_from_introspection(
                    plan.cls_obj, owner_ident=cls_ident, aliases=aliases
                )
                ...
                for r in var_rows:
                    ...
                    if prev is not None:
                        merged = r.copy()
                        if (
                            not (merged.get("doc") or "").strip()
                            and (prev.get("doc") or "").strip()
                        ):
                            merged["doc"] = prev["doc"]
                        if (
                            not (merged.get("type") or "").strip()
                            and (prev.get("type") or "").strip()
                        ):
                            merged["type"] = prev["type"]
                        if (
                            not (merged.get("value") or "").strip()
                            and (prev.get("value") or "").strip()
                        ):
                ...
                derived_map = class_var_info_map_from_ast(
                    py, cls_name, owner_ident=cls_ident
                )
            ...
            for m in plan.methods:
                inv_objects[f"{cls_ident}.{m}"] = (
                    f"{cls_page_url}#{cls_ident}.{m}"
                )
    ...
    disk_path.write_text(
        json.dumps(inv_json, indent=2, sort_keys=True), encoding="utf-8"
    )
...
if __name__ in (
    "__main__",
    "<run_path>",
):  # "<run_path>" is what mkdocs-gen-files' runpy.run_path(...) uses
    main()
