...
class TestQuantumProgram:
    ...
    @pytest.mark.parametrize(
        "state_backend", [QSimQuantumState, STIMQuantumState]
    )
    def test_integrated(self, state_backend):
        ...
        stack = [
            {
                ...
                "state": len(qubits),
                "qubit_labels": qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch Trivial",
                "new_patch_label": "L0",
                "qubits": qubits,
            },  # Autogenerated from patch_types
            {
        ...
        assert (
            len(history) > 0
        )  # Should have multiple frames from the instructions
    ...
    def test_program_serialization_parameterized(self, format, make_temp_path):
        ...
        assert (
            len(new_program_results.shot_histories) == 1
        )  # Should have more frames after running
        ...
        with make_temp_path(suffix=f".{format}") as tempf_path:
            ...
            assert (
                len(loaded_program_results.shot_histories) == 1
            )  # Should have more frames after running
            assert loaded_program.name == "Trivial counter test"
            assert len(loaded_program.instruction_stack) == len(
                stack
...
class TestExecutorParallelism:
    ...
    def test_n_shot_batches_auto_default_dispatches_correct_batch_count(
        ...
        try:
            ...
            shot_counters = {
                i: h[-1]["counter"] for i, h in results.shot_histories.items()
            }
...
class TestResolveInstructionLegacyNameHint:
    ...
    def test_run_with_checkpoint_dir_does_not_create_default_checkpoints(
...
class TestResumeFromCheckpoint:
    ...
    def test_resumed_call_computes_only_missing_shots(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def _count_compute_calls(self, max_frame_limit, seed, shot_index):
                ...
                return original_run_shot_2(
                    self, max_frame_limit, seed, shot_index
                )

            monkeypatch.setattr(
                QuantumProgram, "_run_shot", _count_compute_calls
            )
            ...
            for shot_idx in range(4):
                ...
                history_uninterrupted = results_uninterrupted.shot_histories[
                    shot_idx
                ]
    ...
    def test_resume_with_different_batch_size(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def _count_compute_calls_2(
                self, max_frame_limit, seed, shot_index
            ):
                compute_count_2["n"] += 1
                return original_run_shot(
                    self, max_frame_limit, seed, shot_index
                )

            monkeypatch.setattr(
                QuantumProgram, "_run_shot", _count_compute_calls_2
            )
    ...
    def test_no_content_in_checkpoint_dir_raises_file_exists_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with pytest.raises(
                FileExistsError, match="isn't a recognized checkpoint"
            ):
                program.run(
                    num_shots=2,
                    checkpoint=True,
    ...
    def test_no_content_in_checkpoint_dir_raises_file_exists_error_fresh_run(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for shot_idx in [0, 1, 4, 5]:
                results_worker = ProgramResults(
                    name=f"Worker {shot_idx}", lazy_loading=False
                )
                history = History()
...
class TestResolveShotBatching:
    ...
    def test_resolve_shot_batching_auto_default_with_shot_executor(self):
        ...
        assert (
            n_resolved == 5
        ), f"Expected 5 batches for 100 shots, got {n_resolved}"
        ...
        assert (
            n_resolved == 1
        ), f"Expected 1 batch for 1 shot, got {n_resolved}"
        ...
        assert (
            n_resolved == 2
        ), f"Expected 2 batches for 25 shots, got {n_resolved}"

    def test_resolve_shot_batching_auto_default_checkpoint_batch_size_serial(
        self,
    ):
        ...
        assert (
            batch_size == 5
        ), f"Expected batch size 5 for 100 shots, got {batch_size}"
        ...
        assert (
            batch_size == 1
        ), f"Expected batch size 1 for 1 shot, got {batch_size}"

    def test_resolve_shot_batching_derives_checkpoint_batch_size_from_n_shot_batches(
        self,
    ):
    ...
    def test_resolve_shot_batching_checkpoint_batch_size_with_shot_executor(
        self,
    ):
    ...
    def test_resolve_shot_batching_checkpoint_batch_size_zero_raises_valueerror(
        ...
        with pytest.raises(
            ValueError, match="checkpoint_batch_size must be >= 1"
        ):
            program._resolve_shot_batching(
                num_shots=100,
                shot_executor=object(),
    ...
    def test_resolve_shot_batching_negative_n_shot_batches_raises_valueerror(
        self,
    ):
...
class TestCheckResumeAndResolveCheckpointDir:
    ...
    def test_tolerates_non_quantumprogram_parent_program(self, tmp_path):
        ...
        ), "Should resolve checkpoint_dir successfully"
