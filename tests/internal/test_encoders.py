...
class TestEncoderParameterized:
    ...
    def test_encode_uncached_obj(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                with h5py.File(temp_file, "w") as h5_file:
                    ...
                    encoded_group = HDF5Encoder.encode_uncached_obj(
    ...
    def test_encode_cached_obj(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                with h5py.File(temp_file, "w") as h5_file:
                    root_group = h5_file.create_group("root")
                    encoded_group = HDF5Encoder.encode_cached_obj(
                        cache_id, h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)
                    assert encoded_group.attrs["encode_type"] == "Serializable"
    ...
    def test_decode_cached_obj(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                ...
                with h5py.File(temp_file, "r") as h5_file:
                    ...
                    assert isinstance(encoded_group, h5py.Group)
                    decoded = HDF5Encoder.decode_cached_obj(
                        encoded_group, decode_cache
                    )
                    assert decoded is obj

    def test_encode_iterable(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                with h5py.File(temp_file, "w") as h5_file:
                    root_group = h5_file.create_group("root")
                    encoded_group = HDF5Encoder.encode_iterable(
                        test_list, h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)
                    assert "iterable" in root_group
                    assert encoded_group.attrs["iterable_type"] == "list"

    def test_decode_iterable(self, encoder_format, make_temp_path):
    ...
    def test_encode_dict(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                with h5py.File(temp_file, "w") as h5_file:
                    root_group = h5_file.create_group("root")
                    encoded_group = HDF5Encoder.encode_dict(
                        test_dict, h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)
                    assert "keys" in encoded_group
                    assert "values" in encoded_group

    def test_decode_dict(self, encoder_format, make_temp_path):
    ...
    def test_encode_class(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                with h5py.File(temp_file, "w") as h5_file:
                    root_group = h5_file.create_group("root")
                    encoded_group = HDF5Encoder.encode_class(
                        MockSerializable, h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)
                    assert encoded_group.attrs["module"] == "test_encoders"
                    assert encoded_group.attrs["class"] == "MockSerializable"

    def test_decode_class(self, encoder_format, make_temp_path):
    ...
    def test_encode_function(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                with h5py.File(temp_file, "w") as h5_file:
                    root_group = h5_file.create_group("root")
                    encoded_group = HDF5Encoder.encode_function(
                        test_func, h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)
                    assert "source" in encoded_group

    def test_decode_function(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                with h5py.File(temp_file, "w") as h5_file:
                    ...
                    # Test int
                    encoded_group = HDF5Encoder.encode_primitive(
                        42, h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)

                    # Test float
                    encoded_group = HDF5Encoder.encode_primitive(
                        3.14, h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)

                    # Test string
                    encoded_group = HDF5Encoder.encode_primitive(
                        "hello", h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)
                    ...
                    # Test None
                    encoded_group = HDF5Encoder.encode_primitive(
                        None, h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)

                    # Test special characters
                    encoded_group = HDF5Encoder.encode_primitive(
                        "Café & naïve", h5_group=root_group
                    )
                    assert isinstance(encoded_group, h5py.Group)
    ...
    def test_decode_primitive(self, encoder_format, make_temp_path):
    ...
    class TestEncoderIntegration:
        ...
        def test_encoder_consistency(self, make_temp_path):
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                ...
                with h5py.File(temp_file, "r") as h5_file:
                    ...
                    encoded_group = root_group[obj_group_name]
                    assert isinstance(encoded_group, h5py.Group)
                    hdf5_decoded = HDF5Encoder.decode_uncached_obj(
                        encoded_group
                    )
        ...
        def test_complex_object_roundtrip(self, make_temp_path):
            """Test roundtrip serialization of complex objects with both encoders."""
            complex_obj = MockSerializable(name="complex_test", value=999)

            # Test JSON roundtrip
            json_encoded = JSONEncoder.encode_uncached_obj(complex_obj)
            json_decoded = JSONEncoder.decode_uncached_obj(json_encoded)

            assert isinstance(json_decoded, MockSerializable)
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                with h5py.File(temp_file, "w") as h5_file:
                    ...
                    HDF5Encoder.encode_uncached_obj(
                        complex_obj, h5_group=root_group
                    )
...
class TestComprehensiveArrayEncoding:
    ...
    def test_encode_array_all_types(self, encoder_format, make_temp_path):
                ...
                with h5py.File(temp_file, "w") as h5_file:
                    for i, (name, arr) in enumerate(test_cases):
                        ...
                        if sps.issparse(arr):
                            ...
                            assert encoded.attrs["array_type"] in [
                                "dense_real",
                                "dense_complex",
                            ]
    ...
    def test_decode_array_all_types(self, encoder_format, make_temp_path):
        ...
        if encoder_format == "json":
            ...
            assert isinstance(decoded_sparse, sps.csr_matrix)
            ...
            with make_temp_path(suffix=".h5") as temp_file:
                ...
                with h5py.File(temp_file, "w") as h5_file:
                    root_group = h5_file.create_group("root")
                    HDF5Encoder.encode_array(
                        sps.csr_matrix([[1, 0, 2], [0, 3, 0], [4, 0, 5]]),
                        h5_group=root_group,
                    )
    ...
    def test_array_roundtrip_all_types(self, encoder_format, make_temp_path):
...
class TestHDFIterableOptimizations:
    def test_encode_iterable_hdf5_native_optimization(self, make_temp_path):
    ...
    def test_encode_iterable_bool_int_mix_falls_back_to_groups(
        self, make_temp_path
    ):
    ...
    def test_encode_iterable_mixed_types_fallback(self, make_temp_path):
        ...
        test_list = [
            ...
            "string",
            3.14,
            MockSerializable(name="test", value=42),
        ]
    ...
    def test_encode_iterable_large_dataset_compression(self, make_temp_path):
        ...
        with make_temp_path(suffix=".h5") as temp_file:
            with h5py.File(temp_file, "w") as h5_file:
                root_group = h5_file.create_group("root")
                encoded_group = HDF5Encoder.encode_iterable(
                    large_list, h5_group=root_group
                )
...
class TestHDF5Collapse:
    ...
    def test_fully_array_free_object_collapses_into_one_blob(
        self, make_temp_path
    ):
        ...
        with make_temp_path(suffix=".h5") as temp_file:
            ...
            with h5py.File(temp_file, "r") as h5_file:
                ...
                assert isinstance(
                    obj_group[_COLLAPSED_BLOB_NAME], h5py.Dataset
                )
    ...
    def test_object_with_array_attr_partitions_correctly(self, make_temp_path):
        obj = MockSerializableWithArray(
            name="mixed", data=np.array([1.0, 2.0, 3.0])
        )
    ...
    def test_cached_reference_to_array_containing_object_still_collapses(
        self, make_temp_path
    ):
        ...
        shared = MockSerializableWithArray(
            name="shared", data=np.array([1.0, 2.0])
        )
        same_content_copy = MockSerializableWithArray(
            name="shared", data=np.array([1.0, 2.0])
        )
        ...
        with make_temp_path(suffix=".h5") as temp_file:
            with h5py.File(temp_file, "w") as h5_file:
                ...
                HDF5Encoder.encode_iterable(
                    elements, h5_group=root_group, encode_cache=encode_cache
                )
    ...
    def test_second_reference_to_array_containing_object_collapses_once_registered(
        self, make_temp_path
    ):
    ...
    def test_collapsed_blob_respects_gzip_threshold(self, make_temp_path):
        ...
        with make_temp_path(suffix=".h5") as temp_file:
            ...
            with h5py.File(temp_file, "r") as h5_file:
                assert (
                    h5_file["small"][_COLLAPSED_BLOB_NAME].compression is None
                )
                assert (
                    h5_file["large"][_COLLAPSED_BLOB_NAME].compression
                    == "gzip"
                )
