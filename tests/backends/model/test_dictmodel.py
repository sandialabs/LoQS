...
class TestDictModelV2FixtureRoundTrip:
    """Round-trip `dictmodel_v2.{json,h5}` (frozen, current-code bytes
    generated by `generate_model_fixtures_v2.py`). Unlike
    `TestDictModelFixtureRoundTrip` above, this isn't exercising any
    decode-compatibility redirect today -- it's a `SERIALIZATION_VERSION`
    2 snapshot kept on hand as the reference bytes for whenever the next
    version bump needs one, mirroring the role `dictmodel_v1.{json,h5}`
    played for this version's own `RepTuple`/`STIMDictNoiseModel`
    removal."""

    @pytest.fixture(params=["json", "hdf5"])
    def decoded(self, request):
        if request.param == "json":
            with open(FIXTURES_DIR / "dictmodel_v2.json") as f:
                return Serializable.decode(json.load(f), format="json")
        else:
            with h5py.File(FIXTURES_DIR / "dictmodel_v2.h5", "r") as f:
                return Serializable.decode(f["root"], format="hdf5")
...
class TestDictModelStimV2FixtureRoundTrip:
    ...
    as a plain `DictNoiseModel` since there's no separate
    `STIMDictNoiseModel` class to construct anymore."""

    @pytest.fixture(params=["json", "hdf5"])
    def decoded(self, request):
        if request.param == "json":
            with open(FIXTURES_DIR / "dictmodel_stim_v2.json") as f:
                return Serializable.decode(json.load(f), format="json")
        else:
            with h5py.File(FIXTURES_DIR / "dictmodel_stim_v2.h5", "r") as f:
                return Serializable.decode(f["root"], format="hdf5")
...
class TestSTIMDictNoiseModelDecodeRedirect:
    ...
    straight to `DictNoiseModel` via `IMPORT_LOCATION_CHANGES_BY_VERSION`,
    with no shim class involved in the decode path at all (unlike live
    construction -- see `TestSTIMDictNoiseModelLegacyShim`). This tests
    that decode-time redirect directly."""
...
class TestSTIMDictNoiseModelLegacyShim:
    ...
    kept working via a `make_legacy_construction_shim`, redirecting to a
    plain `DictNoiseModel` with STIM-appropriate `gatereps`/`instreps`
    defaults."""

    def test_importable_from_its_old_module_path(self):
    ...
    def test_construction_from_dict_pair_warns_and_returns_dictnoisemodel(self):
        ...
        with pytest.warns(DeprecationWarning, match="STIMDictNoiseModel"):
            model = STIMDictNoiseModel(({"H": "H", "CX": "CX"}, {"M": "M"}))

        assert type(model) is DictNoiseModel
    ...
    def test_get_reps_works_end_to_end(self):
        ...
        with warnings.catch_warnings():
            ...
            model = STIMDictNoiseModel(({"H": "H 0"}, {"M": "M 0"}))

        circuit = STIMPhysicalCircuit("H 0\nM 0", ["Q0"])
        reps = model.get_reps(
            circuit, [StimCircuitGateRep], [StimCircuitInstrumentRep]
        )
        assert len(reps) == 2
        assert reps[0].circuit_str == "H 0"
        assert isinstance(reps[1], StimCircuitInstrumentRep)

    def test_copy_construction_from_existing_model(self):
        from loqs.backends.model.stimdictmodel import STIMDictNoiseModel

        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DeprecationWarning)
            original = STIMDictNoiseModel(({"H": "H"}, {}))
            copy = STIMDictNoiseModel(original)

        assert copy is not original
        assert copy.gate_dict.keys() == original.gate_dict.keys()

    def test_explicit_reps_and_instrep_cast_kwargs_pass_through(self):
        from loqs.backends.model.stimdictmodel import STIMDictNoiseModel
        ...
        with warnings.catch_warnings():
            ...
            model = STIMDictNoiseModel(
                ...
                instreps=[ZBasisProjectionInstrumentRep],
                instrep_cast_reset=0,
                instrep_cast_include_outcomes=True,
            )

        assert model.output_gate_reps == [StimCircuitGateRep]
        assert model.output_instrument_reps == [ZBasisProjectionInstrumentRep]
        assert isinstance(model.inst_dict["M"], ZBasisProjectionInstrumentRep)
    ...
    def test_invalid_first_argument_raises_type_error(self):
        ...
        with pytest.raises(TypeError, match="2-tuple"):
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", DeprecationWarning)
                STIMDictNoiseModel("not a valid argument")
