...
class TestSTIMDictNoiseModel:
    ...
    def test_circuit_with_comments(self):
        ...
        string, so `STIMDictNoiseModel.get_reps` never actually sees them
        (verified below via `circuit._unroll_repeats()`) -- there is no
        comment-specific code path in `STIMDictNoiseModel` for this test to
        exercise. This documents that fact and confirms `get_reps` still
        produces the expected single-gate result once the comment is gone,
        rather than only checking `len(reps) >= 1`.
        """
        gate_dict = {
            "X": RepTuple("X 0", ("Q0",), GateRep.STIM_CIRCUIT_STR),
        }
        ...
        assert circuit._unroll_repeats() == "X 0", (
            "stim already dropped the comment before STIMDictNoiseModel "
            "ever saw the circuit text"
        )
        ...
        assert len(reps) == 1
        assert reps[0].rep == "X 0"
        assert reps[0].qubits == ("Q0",)

    def test_tuple_key_aliasing(self):
        """Test command aliasing with tuple keys.

        `add_command_aliases` always *adds* the aliased key alongside the
        original rather than replacing it, so both keys must be present
        (not just one or the other) and must resolve to the same
        registered `RepTuple` after construction.
        """
        ...
        assert len(model.gate_dict) == 2
        assert ("CNOT", ("Q0", "Q1")) in model.gate_dict
        assert ("CX", ("Q0", "Q1")) in model.gate_dict
        assert (
            model.gate_dict[("CX", ("Q0", "Q1"))]
            is model.gate_dict[("CNOT", ("Q0", "Q1"))]
        )
...
class TestSTIMDictModelFixtureRoundTrip:
    """Round-trip `tests/backends/model/fixtures/stimdictmodel_v1.{json,h5}`
    ...
    def test_decodes_to_stimdictnoisemodel_with_correct_content(self, decoded):
        # The module/class metadata mechanism must correctly reconstruct
        # STIMDictNoiseModel specifically, not just its base DictNoiseModel
        # -- exactly what a future legacy-decode shim needs to keep working.
        assert isinstance(decoded, STIMDictNoiseModel)
        assert decoded.name == "STIM gate dict"
        # "CX" is present alongside "CNOT" because add_command_aliases ran
        # again during the fixture-generating construction (before it was
        # ever serialized) -- confirms aliasing is captured in the fixture.
        assert set(decoded.gate_dict.keys()) == {"X", "H", "CNOT", "CX"}
        assert decoded.gate_dict["CNOT"].rep == "CNOT 0 1"
        assert decoded.gate_dict["CX"].rep == "CNOT 0 1"
        assert set(decoded.inst_dict.keys()) == {"M"}
        assert decoded.inst_dict["M"].reptype == InstrumentRep.ZBASIS_PROJECTION

    def test_get_reps_works_on_decoded_model(self, decoded):
        """Confirm a decoded model isn't just structurally correct but is
        actually still usable -- i.e. `get_reps` still works end-to-end."""
        circuit = STIMPhysicalCircuit("X 0\nM 0", ["Q0"])
        ...
        assert len(reps) == 2
        assert reps[0].rep == "X 0"
        assert reps[1].reptype == InstrumentRep.ZBASIS_PROJECTION
