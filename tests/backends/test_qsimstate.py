...
class TestQSimQuantumState:
    ...
    def _check(self, state, expected_state):
        ...
        assert np.allclose(
            state.state.full_dm.dm, expected_state.state.full_dm.dm
        )
    ...
    def test_apply_gates(self):
        ...
        test.state.combine_and_apply_single_ptm(
            "Q0"
        )  # Actually force propogation
        self._check(test, state1)

        test2, outcomes = state0.apply_reps(X_reps)
        test2.state.combine_and_apply_single_ptm(
            "Q0"
        )  # Actually force propogation
        ...
        test3.state.combine_and_apply_single_ptm(
            "Q0"
        )  # Actually force propogation
        test3.state.combine_and_apply_single_ptm(
            "Q1"
        ...
        with pytest.raises(NotImplementedError):
            test.apply_reps([UnitaryGateRep(np.eye(2), "Q0")])
        ...
        with pytest.raises(NotImplementedError):
            test.apply_reps([KrausGateRep([(np.eye(2), 1.0)], "Q0")])
    ...
    def test_apply_instruments(self):
        ...
        pre_xpi2_rep = ZBasisPrePostInstrumentRep(
            0, True, xpi2_rep, idle_rep, ["Q0"]
        )
        ...
        post_xpi2_rep = ZBasisPrePostInstrumentRep(
            0, True, idle_rep, xpi2_rep, ["Q0"]
        )
        ...
        test7 = state0.copy()
        outs = test7.apply_reps_inplace(
            [xpi2_rep] + [noisy_reset_map_rep] * 10
        )
        outcomes7 = outs["Q0"]
    ...
    def test_zbasis_outcome_operation_dict_no_outcomes(self):
        ...
        test = QSimState(1, ["Q0"], seed=20260712)
        for _ in range(10):
            outs = test.apply_reps_inplace(
                [xpi2_rep, ideal_map_rep_no_outcomes]
            )
            assert len(outs) == 0
            test.state.combine_and_apply_single_ptm("Q0")
            matches0 = np.allclose(
                test.state.full_dm.dm, state0.state.full_dm.dm
            )
            matches1 = np.allclose(
                test.state.full_dm.dm, state1.state.full_dm.dm
            )
    ...
    def test_zbasis_pre_post_operations_reset_and_no_outcomes(self):
        ...
        pre_reset1_no_outcomes = ZBasisPrePostInstrumentRep(
            1, False, xpi2_rep, idle_rep, ["Q0"]
        )
