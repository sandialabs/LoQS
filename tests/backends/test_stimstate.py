...
class TestSTIMQuantumState:
    ...
    def _check(self, state, expected_state):
        assert (
            state.state.canonical_stabilizers()
            == expected_state.state.canonical_stabilizers()
        )
    ...
    def test_init(self):
        ...
        s_int_labels = STIMState(
            s, qubit_labels=None
        )  # No labels should default to int list
    ...
    def test_apply_gates(self):
        ...
        prob_of_reset = 0.4
        prob_reset_rep = ProbabilisticStimGateRep(
            [("", 1 - prob_of_reset), ("R 0", prob_of_reset)], ["Q0"]
        )
        ...
        expected_outcomes = [
            rng.choice([1, 0], p=[1 - prob_of_reset, prob_of_reset])
            for _ in range(10)
        ]
    ...
    def test_probabilistic_stim_operations_multibranch_multiqubit(self):
        ...
        probs = [0.5, 0.25, 0.25]
        two_qubit_rep = ProbabilisticStimGateRep(
    ...
    def test_apply_instrument_stim_circuit_str(self):
        ...
        state_mrx.apply_reps_inplace([StimCircuitGateRep("RX 0", ["Q0"])])
    ...
    def test_zbasis_pre_post_operations_reset_to_1(self):
        ...
        h_rep = StimCircuitGateRep("H 0", ["Q0"])
        idle_rep = StimCircuitGateRep("", ["Q0"])
        pre_h_reset1_rep = ZBasisPrePostInstrumentRep(
            1, True, h_rep, idle_rep, ["Q0"]
        )
