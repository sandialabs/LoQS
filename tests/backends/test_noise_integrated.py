...
class TestIntegratedNoise:
    ...
    def test_1q_depolarizing(self):
        ...
        qsim_ptm = convert_rep(
            PTMGateRep(ptm, ["Q0"]), QSimSuperopGateRep
        ).superop
        ...
        ## QUANTUMSIM
        gate_dict, inst_dict = self._create_model_dicts(
            qubits, QSimSuperopGateRep
        )
        ...
        outs = [
            mo["Q0"][0]
            for mo in program_results_qsim.collect_shot_data(
        ...
        outs = [
            mo["Q0"][0]
            for mo in program_results_qsim_Xbasis.collect_shot_data(
                "measurement_outcomes", -1
            )
        ]
        ...
        gate_dict[("Gi", ("Q0",))] = (
            f"DEPOLARIZE1({3/4 * p_depol}) 0"  # Depolarizing idle
        )
        ...
        outs = [
            mo["Q0"][0]
            for mo in program_results_stim.collect_shot_data(
                "measurement_outcomes", -1
            )
        ]
        ...
        program_stim_Xbasis = QuantumProgram.from_quantum_program(
            program_stim, stack_Xbasis
        )

        program_results_stim_Xbasis = program_stim_Xbasis.run(num_shots=1000)
        outs = [
            mo["Q0"][0]
            for mo in program_results_stim_Xbasis.collect_shot_data(
                "measurement_outcomes", -1
            )
        ]
    ...
    @pytest.mark.parametrize(
        "p_damp,p_dephase,seed,shots,expected0s,run_stim",
        amp_damp_dephase_tests,
    )
    @pytest.mark.parametrize("stim_rep", [ProbabilisticStimGateRep])
    def test_amp_damp_dephase(
        self, p_damp, p_dephase, seed, shots, expected0s, run_stim, stim_rep
    ):
        ...
        gate_dict[("Gi", ("Q1",))] = (
            qsim_native_ptm  # Amplitude damping/dephasing idle
        )
        ...
        def check(program_results, expected):
            outs = [
                mo["Q1"][0]
                for mo in program_results.collect_shot_data(
        ...
        program_results_qsim_Zprep_Xbasis = program_qsim_Zprep_Xbasis.run(
            num_shots=shots
        )
        ...
        program_stim_Zprep_Xbasis = QuantumProgram.from_quantum_program(
            program_stim, stack_Zprep_Xbasis
        )
        program_results_stim_Zprep_Xbasis = program_stim_Zprep_Xbasis.run(
            num_shots=shots
        )
    ...
    def _create_code(qubits: list[str]):
        ...
        instructions = {}
        for q in qubits:
            instructions[f"X{q[1]}"] = (
                builders.build_physical_circuit_instruction(
                    ListPhysicalCircuit([[("Gx", (q,))]]), name=f"X on {q}"
                )
            )
            instructions[f"H{q[1]}"] = (
                builders.build_physical_circuit_instruction(
                    ListPhysicalCircuit([[("Gh", (q,))]]), name=f"H on {q}"
                )
            )
            instructions[f"I{q[1]}"] = (
                builders.build_physical_circuit_instruction(
                    ListPhysicalCircuit([[("Gi", (q,))]]),
                    name=f"Noisy I on {q}",
                )
            )
            instructions[f"Mz{q[1]}"] = (
                builders.build_physical_circuit_instruction(
                    ListPhysicalCircuit([[("Imrz", (q,))]]), name=f"Mz on {q}"
                )
            )
            instructions[f"Mx{q[1]}"] = (
                builders.build_physical_circuit_instruction(
