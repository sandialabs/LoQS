"""Tester for loqs.codepacks.codepack_5_1_3_quantinuum"""

import pytest

# TODO: Rework to run with native backend only if failure to import
quantumsim = pytest.importorskip("quantumsim")
pygsti = pytest.importorskip("pygsti")
stim = pytest.importorskip("stim")

from loqs.backends import (
    PyGSTiPhysicalCircuit,
    ListPhysicalCircuit,
    PyGSTiNoiseModel,
    DictNoiseModel,
    QSimQuantumState,
    STIMQuantumState,
)
from loqs.backends.reps import QSimSuperopGateRep, StimCircuitGateRep
from loqs.core import Frame, Instruction, QuantumProgram, HistoryDataCollector
from loqs.codepacks import codepack_5_1_3_quantinuum2022 as codepack_5_1_3
from loqs.core.recordables import MeasurementOutcomes
from loqs.tools import fttools


def _check_shot_features(
    results, collect_shot_data_args, expected_outcomes, verbose=False
):
    """Check collected shot features against expected values, replicating
    the removed fttools.test_program_output's pass/fail logic locally so
    this file no longer depends on fttools internals. Each collect_shot_data_args
    entry is a HistoryDataCollector-compatible spec: a plain (key, indices) tuple,
    or a dict also supporting frame_filter to select by log text instead of a
    raw frame index."""
    for args, expected in zip(collect_shot_data_args, expected_outcomes):
        out = HistoryDataCollector.from_raw(args).collect(results)[-1]
        if out != expected:
            if verbose:
                print(f"Output:   {out}")
                print(f"Expected: {expected}")
            return False
    return True


class Test5QCodepack:

    @staticmethod
    def _create_program(circuit_backend, model_backend, state_backend):
        code_5q = codepack_5_1_3.create_qec_code(
            circuit_backend=circuit_backend
        )

        # Define the physical qubit labels for our system here
        qubits = ["A0", "A1"] + [f"D{i+2}" for i in range(5)]

        if state_backend == QSimQuantumState:
            gaterep = QSimSuperopGateRep
        else:
            gaterep = StimCircuitGateRep

        ideal_model = codepack_5_1_3.create_ideal_model(
            qubits, gaterep=gaterep, model_backend=model_backend
        )

        state_type = state_backend
        patch_types = {"5Q": code_5q}

        # Stack
        # The "Init *" instructions will be generated by the QuantumProgram based on the values
        # of state_type and patch_types below
        stack = [
            {
                "instruction": "Init State",
                "state": len(qubits),
                "qubit_labels": qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": qubits,
            },  # Autogenerated from patch_types. Note that 5Q here must be a key into the patch_types dict below
            ("Non-FT Minus Prep", "L0"),
            ("Non-FT Logical X Measure", "L0"),
        ]

        return QuantumProgram(
            stack,
            default_noise_model=ideal_model,
            state_type=state_type,
            patch_types=patch_types,
            name="Prep minus, measure X",
        )

    @classmethod
    def setup_class(cls):
        cls.code_5Q = codepack_5_1_3.create_qec_code(
            circuit_backend=PyGSTiPhysicalCircuit
        )
        cls.program = cls._create_program(
            PyGSTiPhysicalCircuit, PyGSTiNoiseModel, QSimQuantumState
        )
        cls.stim_program = cls._create_program(
            PyGSTiPhysicalCircuit, DictNoiseModel, STIMQuantumState
        )
        cls.qubits = ["A0", "A1"] + [f"D{i+2}" for i in range(5)]

    # Note that tests will be labeled stack_outcome-state-model-circuit with this stacking
    @pytest.mark.parametrize(
        "circuit_backend", [PyGSTiPhysicalCircuit, ListPhysicalCircuit]
    )
    @pytest.mark.parametrize(
        "model_backend", [PyGSTiNoiseModel, DictNoiseModel]
    )
    @pytest.mark.parametrize(
        "state_backend", [QSimQuantumState, STIMQuantumState]
    )
    @pytest.mark.parametrize(
        "stack_outcome",
        [
            ([("Non-FT Logical X Measure", "L0")], 1),
            ([("Z", "L0"), ("Non-FT Logical X Measure", "L0")], 0),
            ([("H", "L0"), ("Non-FT Logical Z Measure", "L0")], 1),
            (
                [("H", "L0"), ("X", "L0"), ("Non-FT Logical Z Measure", "L0")],
                0,
            ),
        ],
    )
    def test_nonft_workflow(
        self, circuit_backend, model_backend, state_backend, stack_outcome
    ):
        if (
            model_backend == PyGSTiNoiseModel
            and state_backend == STIMQuantumState
        ):
            pytest.skip("PyGSTiNoiseModel cannot output STIM reps yet")

        ref_program = self._create_program(
            circuit_backend, model_backend, state_backend
        )

        stack = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },
            ("Non-FT Minus Prep", "L0"),
        ]
        stack += stack_outcome[0]

        program = QuantumProgram.from_quantum_program(ref_program, stack)
        program_results = program.run()
        assert (
            program_results.collect_shot_data("logical_measurement", -1)[0]
            == stack_outcome[1]
        )

    # Note that tests will be labeled stack_outcome-state-model-circuit with this stacking
    @pytest.mark.parametrize(
        "circuit_backend", [PyGSTiPhysicalCircuit, ListPhysicalCircuit]
    )
    @pytest.mark.parametrize(
        "model_backend", [PyGSTiNoiseModel, DictNoiseModel]
    )
    @pytest.mark.parametrize(
        "state_backend", [QSimQuantumState, STIMQuantumState]
    )
    @pytest.mark.parametrize(
        "stack_outcome",
        [
            ([("FT Logical X Measure", "L0")], 1),
            (
                [
                    ("Z", "L0"),
                    ("Flagged QEC", "L0"),
                    ("FT Logical X Measure", "L0"),
                ],
                0,
            ),
        ],
    )
    def test_ft_workflow(
        self, circuit_backend, model_backend, state_backend, stack_outcome
    ):
        if (
            model_backend == PyGSTiNoiseModel
            and state_backend == STIMQuantumState
        ):
            pytest.skip("PyGSTiNoiseModel cannot output STIM reps yet")
        ref_program = self._create_program(
            circuit_backend, model_backend, state_backend
        )

        stack_ft = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },
            ("FT Minus Prep", "L0"),
        ]
        stack_ft += stack_outcome[0]

        program = QuantumProgram.from_quantum_program(ref_program, stack_ft)
        program_results = program.run()
        assert (
            program_results.collect_shot_data("logical_measurement", -1)[0]
            == stack_outcome[1]
        )

    def _test_program(self, program, key, idx):
        noise_injected_programs = (
            fttools.build_discrete_error_injection_programs(
                base_program=program,
                instruction_to_analyze=self.code_5Q.instructions[key],
                stack_idx_to_modify=idx,
                error_circuit_labels=["Gxpi", "Gypi", "Gzpi"],
            )
        )

        runner = fttools.FaultInjectionRunner(
            errored_programs=noise_injected_programs,
            collect_shot_data_args=[("logical_measurement", -1)],
            expected_outcomes=[1],
        )
        failed = runner.run()
        return len(failed) == 0

    def test_all_discrete_errors_prep(self):
        stack_ftprep = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },
            (
                "Non-FT Minus Prep",
                "L0",
            ),  # Make this one error, RUS should catch and rerun it
            ("FT Minus Prep", "L0"),
            ("FT Logical X Measure", "L0"),
        ]

        program_ftprep = QuantumProgram.from_quantum_program(
            self.stim_program, stack_ftprep, name="FT Prep -, FT measure X"
        )
        assert self._test_program(program_ftprep, "Non-FT Minus Prep", 2)

    def test_all_discrete_errors_meas(self):
        ## PART I
        stack_ftmeas_partI = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },  # Autogenerated from patch_types. Note that 5Q here must be a key into the patch_types dict below
            ("FT Minus Prep", "L0"),
            ("FT Logical X Measure Part I Circuit", "L0"),
            ("FT Logical X Measure Part I Feed-Forward", "L0"),
        ]

        program_ftmeas_partI = QuantumProgram.from_quantum_program(
            self.stim_program,
            stack_ftmeas_partI,
            name="FT Prep -, non-FT measure X",
        )
        assert self._test_program(
            program_ftmeas_partI, "FT Logical X Measure Part I Circuit", 3
        )

        ## PART II
        def ideal_partI_apply_fn():
            return Frame({"F1": 0, "inferred_M1": 1})

        ideal_partI = Instruction(
            ideal_partI_apply_fn,
            serialized_apply_fn="",  # Hack, we won't need to serialize
        )

        stack_ftmeas_partII = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },  # Autogenerated from patch_types. Note that 5Q here must be a key into the patch_types dict below
            ("FT Minus Prep", "L0"),
            ("FT Logical X Measure Part I Circuit", "L0"),
            (
                "Ideal Part I Feed-Forward",
                None,
            ),  # Forward reference to global instruction
            ("FT Logical X Measure Part II Circuit", "L0"),
            ("FT Logical X Measure Part II Feed-Forward", "L0"),
        ]

        program_ftmeas_partII = QuantumProgram.from_quantum_program(
            self.stim_program,
            stack_ftmeas_partII,
            global_instructions={"Ideal Part I Feed-Forward": ideal_partI},
            name="FT Prep -, FT measure X",
        )
        assert self._test_program(
            program_ftmeas_partII, "FT Logical X Measure Part II Circuit", 5
        )

        ## PART III
        def ideal_partII_apply_fn():
            return Frame({"F2": 0, "inferred_M2": 1})

        ideal_partII = Instruction(
            ideal_partII_apply_fn,
            serialized_apply_fn="",  # Hack, we won't need to serialize
        )

        stack_ftmeas_partIII = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },  # Autogenerated from patch_types. Note that 5Q here must be a key into the patch_types dict below
            ("FT Minus Prep", "L0"),
            ("FT Logical X Measure Part I Circuit", "L0"),
            (
                "Ideal Part I Feed-Forward",
                None,
            ),  # Forward reference to global instruction
            ("FT Logical X Measure Part II Circuit", "L0"),
            (
                "Ideal Part II Feed-Forward",
                None,
            ),  # Forward reference to global instruction
            ("FT Logical X Measure Part III Circuit", "L0"),
            ("FT Logical X Measure Part III Feed-Forward", "L0"),
        ]

        program_ftmeas_partIII = QuantumProgram.from_quantum_program(
            self.stim_program,
            stack_ftmeas_partIII,
            global_instructions={
                "Ideal Part I Feed-Forward": ideal_partI,
                "Ideal Part II Feed-Forward": ideal_partII,
            },
            name="FT Prep -, FT measure X",
        )
        assert self._test_program(
            program_ftmeas_partIII, "FT Logical X Measure Part III Circuit", 7
        )

    def test_all_discrete_errors_QEC(self):
        # XZZXI
        stack_ftqec1 = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },  # Autogenerated from patch_types. Note that 5Q here must be a key into the patch_types dict below
            ("FT Minus Prep", "L0"),
            ("Flagged XZZXI Check", "L0"),
            ("Flagged XZZXI Feed-Forward", "L0"),
            ("FT Logical X Measure", "L0"),
        ]

        program_ftqec1 = QuantumProgram.from_quantum_program(
            self.stim_program,
            stack_ftqec1,
            name="FT Prep -, QEC, FT measure X",
        )
        assert self._test_program(program_ftqec1, "Flagged XZZXI Check", 3)

        ## IXZZX
        stack_ftqec2 = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },  # Autogenerated from patch_types. Note that 5Q here must be a key into the patch_types dict below
            ("FT Minus Prep", "L0"),
            ("Flagged IXZZX Check", "L0"),
            ("Flagged IXZZX Feed-Forward", "L0"),
            ("FT Logical X Measure", "L0"),
        ]

        program_ftqec2 = QuantumProgram.from_quantum_program(
            self.stim_program,
            stack_ftqec2,
            name="FT Prep -, QEC, FT measure X",
        )
        assert self._test_program(program_ftqec2, "Flagged IXZZX Check", 3)

        ## XIXZZ
        stack_ftqec3 = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },  # Autogenerated from patch_types. Note that 5Q here must be a key into the patch_types dict below
            ("FT Minus Prep", "L0"),
            ("Flagged XIXZZ Check", "L0"),
            ("Flagged XIXZZ Feed-Forward", "L0"),
            ("FT Logical X Measure", "L0"),
        ]

        program_ftqec3 = QuantumProgram.from_quantum_program(
            self.stim_program,
            stack_ftqec3,
            name="FT Prep -, QEC, FT measure X",
        )
        assert self._test_program(program_ftqec3, "Flagged XIXZZ Check", 3)

        ## ZXIXZ
        stack_ftqec4 = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },  # Autogenerated from state_type
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },  # Autogenerated from patch_types. Note that 5Q here must be a key into the patch_types dict below
            ("FT Minus Prep", "L0"),
            ("Flagged ZXIXZ Check", "L0"),
            ("Flagged ZXIXZ Feed-Forward", "L0"),
            ("FT Logical X Measure", "L0"),
        ]

        program_ftqec4 = QuantumProgram.from_quantum_program(
            self.stim_program,
            stack_ftqec4,
            name="FT Prep -, QEC, FT measure X",
        )
        assert self._test_program(program_ftqec4, "Flagged ZXIXZ Check", 3)

    def test_ft_prep_deep_spotcheck(self):
        stack_ftprep_D3X_error = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },
            # Here we inject an X error before layer 2, i.e. between CZs, on qubit 4 (0-indexed, and our qubit convention is 2 aux + 5 data)
            {
                "instruction": "Non-FT Minus Prep + Checks",
                "patch_label": "L0",
                "error_injections": [(2, "Gxpi", 4)],
            },
            # The RUS should do a check, then see that it failed, and do it all again
            {
                "instruction": "FT Minus Prep",
                "patch_label": "L0",
                "error_injections": [(2, "Gxpi", 4)],
            },
            ("FT Logical X Measure", "L0"),
        ]

        program_ftprep_D3X_error = QuantumProgram.from_quantum_program(
            self.program, stack_ftprep_D3X_error
        )
        results = program_ftprep_D3X_error.run()
        assert _check_shot_features(
            results,
            collect_shot_data_args=[
                {
                    "key": "measurement_outcomes",
                    "indices": 0,
                    "frame_filter": {
                        "log": "Non-FT Minus Prep + Checks result"
                    },
                },  # Let's grab the outcomes for the first round of RUS
                {
                    "key": "rus_success",
                    "indices": 0,
                    "frame_filter": {
                        "log": "Repeat-until-success FT Minus Prep result"
                    },
                },  # And also the computed RUS success value
                {
                    "key": "measurement_outcomes",
                    "indices": -1,
                    "frame_filter": {
                        "log": "Non-FT Minus Prep + Checks result"
                    },
                },  # Let's do it again for the second round
                {
                    "key": "rus_success",
                    "indices": -1,
                    "frame_filter": {
                        "log": "Repeat-until-success FT Minus Prep result"
                    },
                },
                (
                    "logical_measurement",
                    -1,
                ),  # And still check the logical outcome
            ],
            expected_outcomes=[
                MeasurementOutcomes({"A0": [0, 0, 1], "A1": [0, 0, 0]}),
                False,
                MeasurementOutcomes({"A0": [0, 0, 0], "A1": [0, 0, 0]}),
                True,
                1,
            ],
        )

    def test_ft_meas_deep_spotcheck(self):
        stack_ftmeas_partI_D2Z_error = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },
            ("FT Minus Prep", "L0"),
            # Insert at layer 0, a Z error, on qubit 3 (0-indexed, where first 2 are aux qubits)
            {
                "instruction": "FT Logical X Measure Part I Circuit",
                "patch_label": "L0",
                "error_injections": [(0, "Gzpi", 3)],
            },
            ("FT Logical X Measure Part I Feed-Forward", "L0"),
        ]

        program_ftmeas_partI_D2Z_error = QuantumProgram.from_quantum_program(
            self.program, stack_ftmeas_partI_D2Z_error
        )
        results = program_ftmeas_partI_D2Z_error.run()
        assert _check_shot_features(
            results,
            collect_shot_data_args=[
                {
                    "key": "measurement_outcomes",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part I Circuit result"
                    },
                },  # This should be all 0
                {
                    "key": "F1",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part I Feed-Forward result"
                    },
                },  # Should have no flag
                {
                    "key": "inferred_M1",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part I Feed-Forward result"
                    },
                },  # and this should have correctly identified logical 1 measurement
                {
                    "key": "measurement_outcomes",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part II Circuit result"
                    },
                },  # Now syndrome should be tripped
                {
                    "key": "F2",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part II Feed-Forward result"
                    },
                },  # Should have no flag
                {
                    "key": "inferred_M2",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part II Feed-Forward result"
                    },
                },  # but incorrectly identify logical 0 measurement
                {
                    "key": "measurement_outcomes",
                    "indices": -1,
                    "frame_filter": {
                        "log": "Non-FT state decoder circuit result"
                    },
                },  # The Z error should show up as a bitflip on data qubit 2
                {
                    "key": "classical_correction",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Classical Decoder result"
                    },
                },  # Remember that this is the decoded error
                {
                    "key": "corrected_outcomes",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Classical Decoder result"
                    },
                },  # ... and applied the correction properly
                (
                    "logical_measurement",
                    -1,
                ),  # And still check the logical outcome
            ],
            expected_outcomes=[
                MeasurementOutcomes({"A0": [0], "A1": [0]}),
                0,
                1,
                MeasurementOutcomes({"A0": [1], "A1": [0]}),
                0,
                0,
                MeasurementOutcomes(
                    {"D2": [0], "D3": [1], "D4": [0], "D5": [0], "D6": [0]}
                ),
                "IXIII",
                [0, 0, 0, 0, 0],
                1,
            ],
            verbose=True,
        )

        stack_ftmeas_partI_M1X_error = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },
            ("FT Minus Prep", "L0"),
            # Insert at layer 3, a X error, on qubit 0 (0-indexed, where first qubit is measure qubit)
            {
                "instruction": "FT Logical X Measure Part I Circuit",
                "patch_label": "L0",
                "error_injections": [(3, "Gxpi", 0)],
            },
            ("FT Logical X Measure Part I Feed-Forward", "L0"),
        ]

        program_ftmeas_partI_M1X_error = QuantumProgram.from_quantum_program(
            self.program, stack_ftmeas_partI_M1X_error
        )
        results = program_ftmeas_partI_M1X_error.run()
        assert _check_shot_features(
            results,
            collect_shot_data_args=[
                {
                    "key": "measurement_outcomes",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part I Circuit result"
                    },
                },  # An X error is undetectable on measure, but it should trip the flag!
                {
                    "key": "F1",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part I Feed-Forward result"
                    },
                },  # We should have a flag...
                {
                    "key": "inferred_M1",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Part I Feed-Forward result"
                    },
                },  # ... but still think this is logical 1
                # The expected measurement outcome is non-trivial.
                # The X error on data qubit 1 will prop to Z errors on data qubit 2 and 5
                # This will cancel the Z error on data qubit 2, but show up as a bitflip on data qubit 5
                {
                    "key": "measurement_outcomes",
                    "indices": -1,
                    "frame_filter": {
                        "log": "Non-FT state decoder circuit result"
                    },
                },
                {
                    "key": "classical_correction",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Classical Decoder result"
                    },
                },  # Remember that this is the decoded error
                {
                    "key": "corrected_outcomes",
                    "indices": -1,
                    "frame_filter": {
                        "log": "FT Logical X Measure Classical Decoder result"
                    },
                },
                (
                    "logical_measurement",
                    -1,
                ),  # And still check the logical outcome
            ],
            expected_outcomes=[
                MeasurementOutcomes({"A0": [0], "A1": [1]}),
                1,
                1,
                MeasurementOutcomes(
                    {"D2": [0], "D3": [0], "D4": [0], "D5": [0], "D6": [1]}
                ),
                "IIIIX",
                [0, 0, 0, 0, 0],
                1,
            ],
        )

    def test_h_syndrome_permutation(self):
        ref_program = self._create_program(
            PyGSTiPhysicalCircuit, DictNoiseModel, STIMQuantumState
        )
        stack = [
            {
                "instruction": "Init State",
                "state": len(self.qubits),
                "qubit_labels": self.qubits,
            },
            {
                "instruction": "Init Patch 5Q",
                "new_patch_label": "L0",
                "qubits": self.qubits,
            },
            {
                "instruction": "Non-FT Minus Prep",
                "patch_label": "L0",
                "error_injections": [(2, "Gzpi", 2)],
            },
            ("Unflagged QEC", "L0"),
            ("H", "L0"),
        ]
        program = QuantumProgram.from_quantum_program(ref_program, stack)
        results = program.run()
        history = results.shot_histories[0]
        h_frame = history[-1]
        assert "raw_syndrome" in h_frame
        assert h_frame["raw_syndrome"] == [0, 1, 1, 0]
